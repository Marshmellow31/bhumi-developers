    ...
    >
        ...
        <div className="relative h-full flex flex-col justify-between p-6 md:p-8">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className={`text-[10px] tracking-[0.12em] uppercase font-semibold px-2.5 py-1 font-body ${
                project.status === "Ongoing" ? "bg-amber-500 text-white"
          ...
          <div className="flex items-end justify-between gap-4">
            <div>
              <h2 className="text-2xl md:text-3xl font-bold text-white leading-tight mb-2" style={{ fontFamily: "var(--font-heading)" }}>
                {project.name}
              </h2>
              <p className="text-white/45 text-xs italic font-body mb-2">&ldquo;{project.tagline}&rdquo;</p>
              <div className="flex items-center gap-1.5 text-white/50 text-xs font-body">
                <MapPin size={11} className="text-amber-400 shrink-0" />
                <span>{project.location}</span>
              </div>
            </div>
            <Link href={`/projects/${project.slug}`} className="shrink-0 hidden sm:flex items-center gap-1.5 text-[11px] tracking-[0.1em] uppercase font-semibold font-body text-white/55 hover:text-white transition-colors duration-200 group/lnk" data-cursor-label="VIEW">
              View Project
              <ChevronRight size={13} className="transition-transform duration-200 group-hover/lnk:translate-x-0.5" />
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 md:p-8 bg-[#FAFAFA]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {project.brochure && (
            <DocCard name="Complete Project Brochure" meta="Official Digital Brochure • PDF" label="Brochure" file={project.brochure} delay={0} />
          )}
          {project.floorPlans?.map((plan, i) => (
            <DocCard key={i} name={plan.name} meta={`Floor Plan / Layout${plan.size ? ` • ${plan.size}` : " • PDF"}`} label="Floor Plan" file={plan.file} delay={(i + (project.brochure ? 1 : 0)) * 0.04} />
          ))}
        </div>
      </div>
    </motion.div>
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <h4 className="text-[12px] font-bold text-primary font-body leading-snug line-clamp-2">{name}</h4>
          <p className="text-[10px] text-muted font-body mt-0.5 leading-relaxed">{meta}</p>
        </div>
      </div>
